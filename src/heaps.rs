...
pub struct HeapIndexed<T: NdFloat> {
    pub heap: Vec<(T, usize)>,
    positions: Vec<Option<usize>>,
    is_max_heap: bool,
}

impl<T: NdFloat> HeapIndexed<T> {
    ...
    pub fn push(&mut self, value: T, idx: usize) {
    ...
    pub fn pop(&mut self) -> Option<(T, usize)> {
        ...
        let result: (T, usize) = self.heap[0];
        self.positions[result.1] = None;

        let last: (T, usize) = self.heap.pop().unwrap();
    ...
    fn sift_down(&mut self, mut pos: usize) {
        ...
        let node_value: T = self.heap[pos].0;
...
pub struct IndexedProcessor<T: NdFloat> {
    pub small_heap: HeapIndexed<T>,
    pub large_heap: HeapIndexed<T>,
    pub deque: VecDeque<(T, usize)>,
}
impl<T: NdFloat> IndexedProcessor<T> {
    ...
    pub fn remove(&mut self, window: &mut WindowState<T>, length: usize) {
    ...
    pub fn get(&self) -> T {
        (self.small_heap.last().add(self.large_heap.last())).div(T::one().add(T::one()))
    }
}
