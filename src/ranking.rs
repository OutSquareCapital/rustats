...
pub fn move_rank<'py>(
    ...
    if parallel {
        move_rank_parallel(py, array, length, min_length)
    } else {
        move_rank_single(py, array, length, min_length)
    }
}

fn move_rank_parallel<'py>(
    py: Python<'py>,
    array: PyReadonlyArray2<'py, f64>,
    length: usize,
    min_length: usize
...
pub fn agg_rank<'py>(
    ...
    py.allow_threads(move || {
        input_columns
            ...
            .zip(output_columns.par_iter_mut())
            .for_each(|(input_col, output_col)| {
                let mut value_indices: Vec<(f64, usize)> = Vec::new();
