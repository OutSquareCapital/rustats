...
def save_group_passes(
    ...
    old_data = pl.scan_ndjson(Files.PASSES, schema=Schemas.PASSES)
    ...
    (
        ...
        .sort(
            by=[ColNames.GROUP, ColNames.VERSION, ColNames.TIME_TARGET],
            descending=[False, True, True],
        )
        .unique(subset=[ColNames.GROUP], keep="first")
        .sort([ColNames.GROUP])
        .collect()
        .write_ndjson(Files.PASSES)
    )
